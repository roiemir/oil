    ...
    Parser.prototype = {
        ...
        conditional: function(first) {
            ...
            if (this.expect('?')) {
                ...
                if (first && (this.tokens.length === 0 || this.expect(')'))) {
                    return { "!exp": "?:", expression: expression, positive: positive, negative: null };
                }
        ...
        selector: function (sequence) {
            ...
            if (this.tokens.length > 2 && this.tokens[0].identifier) {
                ...
                if (op === "," && this.tokens.length > 4) {
                    idx = true;
                    op = this.tokens[3].text;
                }
                if (op === "?" || op === "->" || op === "=>" || op === "~>" || op === "~" || op === "~=") {
                    ...
                    if (op === "?") {
                        ...
                        if (this.expect(":")) {
                            ...
                            return {"!exp": "s", sequence: sequence, key: key.identifier, index: idx ? idx.identifier : undefined, condition: condition };
                        }
                    }
                    ...
                    if (op === "~" || grouped) {
                        ...
                        if (op = this.expect("->", "=>", "~>")) {
                            ...
                            return {"!exp": "s", sequence: sequence, key: key.identifier, index: idx ? idx.identifier : undefined, group: order};
                        }
                        else {
                            return {"!exp": "s", sequence: sequence, key: key.identifier, index: idx ? idx.identifier : undefined, order: order };
                        }
                    }

                    if (op) {
                        var selection = this.expression();
                        var result = {"!exp": "s", sequence: sequence, key: key.identifier, index: idx ? idx.identifier : undefined, selection: selection };
                        if (condition) {
